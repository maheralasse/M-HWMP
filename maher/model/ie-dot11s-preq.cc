...
IePreq::IePreq () :
  ...
  m_destCount (0),m_processing_delay_required (0),m_bandwidth_required (0)                             //////////////////
{
}
...
void
IePreq::SetProcessing_delay_required (uint8_t processing_delay_required)          //////////////////
{
  m_processing_delay_required = processing_delay_required;
}

void
IePreq::SetBandwidth_required (uint8_t bandwidth_required)                   //////////////////
{
  m_bandwidth_required = bandwidth_required;
}
...
uint8_t                
IePreq::GetProcessing_delay_required () const                  //////////////////
{
  return m_processing_delay_required;
}


uint8_t
IePreq::GetBandwidth_required () const                             //////////////////
{
  return m_bandwidth_required;
}
...
IePreq::SerializeInformationField (Buffer::Iterator i) const
{
  ...
  i.WriteU8(m_processing_delay_required);            //////////////////
  i.WriteU8(m_bandwidth_required);                    //////////////////
  int written = 0;
...
IePreq::DeserializeInformationField (Buffer::Iterator start, uint8_t length)
{
  ...
  m_flags = i.ReadU8 ();
  m_hopCount = i.ReadU8 ();
  m_bandwidth_required = i.ReadU8 ();
  m_processing_delay_required = i.ReadU8 ();
  m_ttl = i.ReadU8 ();
...
IePreq::Print (std::ostream &os) const
{
  os << "PREQ=(originator address=" << m_originatorAddress
     ...
     << ", lifetime=" << m_lifetime
     << ", preq ID=" << m_preqId
     << ", Processing Delay Required=" << m_processing_delay_required        ////////////
     << ", Bandwidth Required=" << m_bandwidth_required                      ////////////
     << ", Destinations=(";
  for (int j = 0; j < m_destCount; j++)
    {
...
operator== (const IePreq & a, const IePreq & b)
{
  bool ok = (a.m_flags == b.m_flags && a.m_hopCount == b.m_hopCount && a.m_ttl == b.m_ttl && a.m_preqId
    ...
    == b.m_destCount && a.m_processing_delay_required == b.m_processing_delay_required && a.m_bandwidth_required == b.m_bandwidth_required);////////             
