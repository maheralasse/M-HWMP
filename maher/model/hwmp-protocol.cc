...
#include "ns3/maher-point-device.h"
#include "ns3/wifi-net-device.h"
#include "ns3/maher-point-device.h"
#include "ns3/maher-wifi-interface-mac.h"
...
void
HwmpProtocol::ReceivePreq (IePreq preq, Mac48Address from, uint32_t interface, Mac48Address fromMp, uint32_t metric,uint8_t processing_delay_required,uint8_t bandwidth_required)
{
  uint32_t bytesToRequest = 0;            /////////////////////
  uint8_t bandwidth_required=0;
  uint8_t processing_delay_required=0;           

  Time Tcurrent = Simulator::Now ();                  /////////////////////
  ...
  std::vector<Ptr<DestinationAddressUnit> > destinations = preq.GetDestinationList ();
	...
	Ptr<FlowMonitor> monitor = flowmon.InstallAll();
		...
		{
	bytesToRequest = classing->FindFlow (stats->first);                                                                           
                 }
             bandwidth_required=flowmon.BandwidthCalculator(bytesToRequest);
                Time Tstarttransmit = Simulator::Now ();                               /////////////////////

                processing_delay_required=Tcurrent-Tstarttransmit;
  ...
  if (
    ...
    (
      ...
      (m_rtable->LookupReactive (preq.GetOriginatorAddress ()).processing_delay_required < preq.GetProcessing_delay_required ())&&
      (m_rtable->LookupReactive (preq.GetOriginatorAddress ())).bandwidth_required >= preq.GetBandwidth_required ()) ///////
    )
    )
    {
      m_rtable->AddReactivePath (
        ...
        preq.GetProcessing_delay_required (),                                /////////////////////
        preq.GetBandwidth_required ()                                       /////////////////////
        );        
      ...
      if ((*i)->GetDestinationAddress () == Mac48Address::GetBroadcast ())
        {
          ...
          ((m_rtable->LookupProactive (preq.GetOriginatorAddress ())).processing_delay_required < preq.GetProcessing_delay_required ())&&                          
          ((m_rtable->LookupProactive (preq.GetOriginatorAddress ())).bandwidth_required >= preq.GetBandwidth_required ()) ////////
            )
            {
...
HwmpProtocol::ReceivePrep (IePrep prep, Mac48Address from, uint32_t interface, Mac48Address fromMp, uint32_t metric,uint8_t processing_delay_required,uint8_t bandwidth_required)
{
  ...
  if (
    ...
    (
      ...
      ((m_rtable->LookupReactive (prep.GetOriginatorAddress ())).processing_delay_required < prep.GetProcessing_delay_required ())&&                          
      ((m_rtable->LookupReactive (prep.GetOriginatorAddress ())).bandwidth_required >= prep.GetBandwidth_required ())  //////                                      
    )
    )
    {
      m_rtable->AddReactivePath (
        ...
        sequence,
        prep.GetProcessing_delay_required (),
        prep.GetBandwidth_required ()
);
...
void
HwmpProtocol::ReceivePerr (std::vector<FailedDestination> destinations, Mac48Address from, uint32_t interface, Mac48Address fromMp,uint8_t processing_delay_required,uint8_t bandwidth_required)
{
  //Acceptance cretirea:
...
)
{
  ...
  prep.SetBandwidth_required (bandwidth_required);                            /////////////////////
  prep.SetProcessing_delay_required (processing_delay_required);                         /////////////////////
