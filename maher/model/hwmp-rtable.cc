...
HwmpRtable::AddReactivePath (Mac48Address destination, Mac48Address retransmitter, uint32_t interface,
                             uint32_t metric, Time lifetime, uint32_t seqnum, uint8_t m_processing_delay_required, uint8_t m_bandwidth_required) ///////            
{
 ...
 if (i == m_routes.end ())
  ...
  i->second.m_processing_delay_required = m_processing_delay_required;    ////////////////////////
  i->second.m_bandwidth_required = m_bandwidth_required;                      //////////////////
}
void
HwmpRtable::AddProactivePath (uint32_t metric, Mac48Address root, Mac48Address retransmitter,
                              uint32_t interface, Time lifetime, uint32_t seqnum, uint8_t processing_delay_required, uint8_t bandwidth_required)//////////////
{
  m_root.root = root;
